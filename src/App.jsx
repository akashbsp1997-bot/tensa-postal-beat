...
// ─── Derived from config ──────────────────────────────────────────────────────
const STATUS_COLOR = Object.fromEntries(CONFIG.statuses.map(s => [s.id, s.color]));
const COLOR_MAP    = Object.fromEntries(CONFIG.markerColors.map(c => [c.id, c.hex]));
const AREA_DEFAULT_COORDS = Object.fromEntries(CONFIG.areas.map(a => [a.name, [a.lat, a.lng]]));
...
async function getDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      ["areas","houses","businesses","followups","appconfig"].forEach(s => {
        if (!db.objectStoreNames.contains(s)) {
          const store = db.createObjectStore(s, { keyPath: "id" });
          if (s === "houses" || s === "businesses") store.createIndex("by-area", "areaId");
          if (s === "followups") store.createIndex("by-date", "followUpDate");
        }
      });
    };
    req.onsuccess = async (e) => {
      ...
      if (count === 0) {
        CONFIG.areas.forEach(a => store.add({ id: crypto.randomUUID(), name: a.name, _lat: a.lat, _lng: a.lng }));
      }
...
async function dbGet(store, id) {
  const db = await getDB();
  return new Promise((res,rej) => { const r = db.transaction(store,"readonly").objectStore(store).get(id); r.onsuccess=()=>res(r.result); r.onerror=()=>rej(r.error); });
}
async function dbPut(store, obj) {
  const db = await getDB();
  return new Promise((res,rej) => { const r = db.transaction(store,"readwrite").objectStore(store).put(obj); r.onsuccess=()=>res(r.result); r.onerror=()=>rej(r.error); });
}
...
async function dbExport() {
  const [areas,houses,businesses,followups] = await Promise.all([dbGetAll("areas"),dbGetAll("houses"),dbGetAll("businesses"),dbGetAll("followups")]);
...
async function dbImport(json) {
  ...
  for (const s of stores) { tx.objectStore(s).clear(); for (const item of (data[s]||[])) tx.objectStore(s).put(item); }
  return new Promise((res,rej) => { tx.oncomplete=res; tx.onerror=rej; });
}

// Load admin overrides from IndexedDB and merge with CONFIG
async function loadAppConfig() {
  try {
    const overrides = await dbGet("appconfig","main");
    if (!overrides) return CONFIG;
    return { ...CONFIG, app: { ...CONFIG.app, ...overrides.app }, features: { ...CONFIG.features, ...overrides.features } };
  } catch { return CONFIG; }
}
    ...
    if (focusAreaId) {
      ...
      if (areaHouses.length > 0) {
        ...
        areaHouses.forEach(h => {
          ...
          marker.bindPopup(`<div style="min-width:180px;font-family:system-ui;padding:4px 0"><div style="font-weight:800;font-size:14px;margin-bottom:4px">${h.ownerName||"Unknown"}</div><div style="font-size:12px;color:#64748b;margin-bottom:6px">House ${h.houseNumber||""}</div><div style="margin-bottom:8px"><span style="background:${STATUS_COLOR[h.customerStatus]||"#888"}22;color:${STATUS_COLOR[h.customerStatus]||"#888"};padding:2px 8px;border-radius:5px;font-size:11px;font-weight:700;text-transform:capitalize">${h.customerStatus}</span></div>${h.mobileNumber?`<a href="tel:${h.mobileNumber}" style="display:block;background:#4f6ef7;color:#fff;text-align:center;padding:7px;border-radius:8px;text-decoration:none;font-weight:700;font-size:13px">📞 ${h.mobileNumber}</a>`:""}</div>`,{maxWidth:220});
          marker.addTo(map); markersRef.current.push(marker); bounds.push([h.gpsLat,h.gpsLng]);
        });
        if (bounds.length>0) map.fitBounds(bounds,{padding:[40,40]});
      } else {
        const pos = area?._lat&&area?._lng ? [area._lat,area._lng] : (AREA_DEFAULT_COORDS[area?.name]||[22.015,85.196]);
        map.setView(pos,16);
      ...
      areas.forEach(area => {
        ...
        let lat,lng;
        if (area._lat&&area._lng) { lat=area._lat; lng=area._lng; }
        else if (gpsHouses.length>0) { lat=gpsHouses.reduce((s,h)=>s+h.gpsLat,0)/gpsHouses.length; lng=gpsHouses.reduce((s,h)=>s+h.gpsLng,0)/gpsHouses.length; }
        else { const def=AREA_DEFAULT_COORDS[area.name]; if(!def) return; [lat,lng]=def; }
        const count = areaHouses.length;
        const drag = !!editMode;
        const icon = L.divIcon({ html:`<div style="background:${drag?"#f97316":"#4f6ef7"};color:#fff;border-radius:20px;padding:6px 10px;font-weight:800;font-size:12px;box-shadow:0 2px 8px rgba(0,0,0,.35);white-space:nowrap;border:2px solid #fff;cursor:${drag?"grab":"pointer"}">${drag?"✥ ":""}${area.name.replace(" Area","")}<br><span style="font-size:10px;font-weight:400;opacity:.85">${drag?"drag to move":`${count} ${count===1?"house":"houses"}`}</span></div>`, className:"", iconAnchor:[0,0] });
...
export default function App() {
  ...
  useEffect(()=>{ loadAppConfig().then(setCfg); },[refresh]);
  ...
  const theme = {
    ...
    primary: cfg.app.primaryColor||"#4f6ef7",
    accent:  cfg.app.accentColor||"#f97316",
  ...
  const navItems = [
    ...
    f.map        && { id:"map",      icon:"🗺️",  label:"Map" },
    { id:"add",       icon:"➕",  label:"Add" },
    f.search     && { id:"search",   icon:"🔍",  label:"Search" },
    f.reports    && { id:"reports",  icon:"📊",  label:"Reports" },
  ].filter(Boolean);

  const pages = {
    dashboard: <Dashboard nav={nav} theme={theme} refresh={refresh} cfg={cfg} />,
    map:       <MapPage nav={nav} theme={theme} refresh={refresh} />,
    add:       <AddEntry nav={nav} theme={theme} params={pageParams} toast={toast} bump={bump} cfg={cfg} />,
    ...
    settings:  <SettingsPage nav={nav} theme={theme} dark={dark} setDark={setDark} toast={toast} bump={bump} cfg={cfg} />,
    admin:     <AdminPage nav={nav} theme={theme} toast={toast} bump={bump} cfg={cfg} />,
  ...
  const hideNav = ["add","house-detail","biz-detail","followups","settings","map","admin"].includes(page);
...
// ─── Admin Page ───────────────────────────────────────────────────────────────
function AdminPage({ nav, theme, toast, bump, cfg }) {
  const [app, setApp] = useState({ ...cfg.app });
  const [features, setFeatures] = useState({ ...cfg.features });
  const [saving, setSaving] = useState(false);

  const save = async () => {
    setSaving(true);
    await dbPut("appconfig", { id:"main", app, features });
    toast("Settings saved! Restart app to see all changes."); bump(); setSaving(false);
  };
  const reset = async () => {
    if (!confirm("Reset all admin settings to config.js defaults?")) return;
    await dbDelete("appconfig","main"); toast("Reset to defaults!"); bump(); nav("settings");
  };

  const featureLabels = {
    map:"Map Page", businesses:"Businesses", followups:"Follow-ups",
    reports:"Reports", photoCapture:"Photo Capture", gpsCapture:"GPS Capture",
    csvExport:"CSV Export", darkMode:"Dark Mode", search:"Search",
  };

  return (
    <div>
      <PageHeader title="⚙️ Admin Panel" onBack={()=>nav("settings")} theme={theme} />
      <div style={{padding:"0 16px 100px",display:"flex",flexDirection:"column",gap:20}}>

        <Sec title="APP BRANDING" theme={theme}>
          <div style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:10}}>
            <FI label="App Title" value={app.title} onChange={v=>setApp(a=>({...a,title:v}))} theme={theme} />
            <FI label="Subtitle" value={app.subtitle} onChange={v=>setApp(a=>({...a,subtitle:v}))} theme={theme} />
            <FI label="Icon (emoji)" value={app.icon} onChange={v=>setApp(a=>({...a,icon:v}))} theme={theme} />
            <div>
              <div style={{fontSize:12,fontWeight:700,color:theme.muted,marginBottom:8}}>PRIMARY COLOR</div>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <input type="color" value={app.primaryColor} onChange={e=>setApp(a=>({...a,primaryColor:e.target.value}))} style={{width:44,height:44,borderRadius:8,border:"none",cursor:"pointer"}} />
                <span style={{fontSize:13,color:theme.muted}}>{app.primaryColor}</span>
              </div>
            </div>
            <div>
              <div style={{fontSize:12,fontWeight:700,color:theme.muted,marginBottom:8}}>ACCENT COLOR</div>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <input type="color" value={app.accentColor} onChange={e=>setApp(a=>({...a,accentColor:e.target.value}))} style={{width:44,height:44,borderRadius:8,border:"none",cursor:"pointer"}} />
                <span style={{fontSize:13,color:theme.muted}}>{app.accentColor}</span>
              </div>
            </div>
          </div>
        </Sec>

        <Sec title="FEATURES" theme={theme}>
          {Object.entries(featureLabels).map(([key,label],i,arr)=>(
            <div key={key} style={{padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:i<arr.length-1?`1px solid ${theme.border}`:"none"}}>
              <span style={{fontSize:14,color:theme.text,fontWeight:500}}>{label}</span>
              <Toggle checked={!!features[key]} onChange={v=>setFeatures(f=>({...f,[key]:v}))} theme={theme} />
            </div>
          ))}
        </Sec>

        <SaveBtn label="Save Admin Settings" color={theme.primary} onClick={save} saving={saving} />

        <button onClick={reset} style={{width:"100%",padding:"14px",borderRadius:12,border:`1px solid ${theme.danger}`,background:"transparent",color:theme.danger,fontSize:14,fontWeight:700,cursor:"pointer"}}>
          Reset to Config Defaults
        </button>

        <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:12,padding:14}}>
          <div style={{fontSize:12,fontWeight:700,color:theme.muted,marginBottom:6}}>TIP — config.js</div>
          <div style={{fontSize:12,color:theme.muted,lineHeight:1.6}}>
            For deeper changes (area names, statuses, marker colors, services) edit <span style={{fontFamily:"monospace",background:theme.bg,padding:"1px 5px",borderRadius:4}}>src/config.js</span> directly on GitHub. It auto-deploys in ~60 seconds.
          </div>
        </div>
      </div>
    </div>
...
// simple inline field for admin
function FI({ label, value, onChange, theme }) {
  return (
    <div>
      <div style={{fontSize:11,fontWeight:700,color:theme.muted,marginBottom:4}}>{label}</div>
      <input value={value||""} onChange={e=>onChange(e.target.value)} style={{width:"100%",padding:"10px",borderRadius:8,border:`1px solid ${theme.border}`,background:theme.bg,color:theme.text,fontSize:14,outline:"none",boxSizing:"border-box"}} />
    </div>
  );
}
...
function MapPage({ nav, theme, refresh }) {
  ...
  useEffect(()=>{ Promise.all([dbGetAll("areas"),dbGetAll("houses")]).then(([a,h])=>{setAreas(a);setHouses(h);}); },[refresh]);
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh"}}>
        ...
        {focusArea ? (
          ...
          <>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:theme.muted}}>{editMode?"Drag markers to reposition":"Tap an area to see houses"}</div>
            </div>
      ...
      {editMode&&!focusArea&&(
        <div style={{background:"#f97316",color:"#fff",padding:"8px 16px",fontSize:12,fontWeight:600,textAlign:"center",flexShrink:0}}>
          {savedMsg?"✓ Position saved!":"Drag any area marker to reposition — saves automatically"}
        </div>
      )}
      {focusArea&&(
        <div style={{background:theme.card,borderBottom:`1px solid ${theme.border}`,padding:"8px 16px",display:"flex",gap:12,flexShrink:0,overflowX:"auto"}}>
          {CONFIG.markerColors.map(c=>(
            <div key={c.id} style={{display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}>
              <div style={{width:10,height:10,borderRadius:"50%",background:c.hex}}/>
              <span style={{fontSize:10,color:theme.muted,textTransform:"capitalize"}}>{c.id}</span>
            </div>
          ))}
        </div>
      )}
...
// ─── Dashboard ────────────────────────────────────────────────────────────────
function Dashboard({ nav, theme, refresh, cfg }) {
  const [data,setData]=useState({houses:[],businesses:[],areas:[]});
  const [loading,setLoading]=useState(true);
  useEffect(()=>{ Promise.all([dbGetAll("houses"),dbGetAll("businesses"),dbGetAll("areas")]).then(([houses,businesses,areas])=>{setData({houses,businesses,areas});setLoading(false);}).catch(()=>setLoading(false)); },[refresh]);
  ...
  const stats=[
    ...
    {label:"Today",value:houses.filter(h=>h.createdAt?.startsWith(today)).length+businesses.filter(b=>b.createdAt?.startsWith(today)).length,icon:"📝"},
  ];
  const recent=[...houses.sort((a,b)=>b.createdAt?.localeCompare(a.createdAt||"")).slice(0,3),...businesses.sort((a,b)=>b.createdAt?.localeCompare(a.createdAt||"")).slice(0,2)];
  return (
    <div style={{padding:"16px 16px 8px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div>
          <div style={{fontSize:24,fontWeight:800,color:theme.primary,letterSpacing:-0.5}}>{cfg.app.icon} {cfg.app.title}</div>
          <div style={{fontSize:12,color:theme.muted,marginTop:2}}>{cfg.app.subtitle}</div>
        </div>
      ...
      <div style={{marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          {cfg.features.businesses&&<QuickBtn theme={theme} color={theme.accent} icon="🏪" label="Add Business" onClick={()=>nav("add",{type:"business"})} />}
          {cfg.features.followups&&<QuickBtn theme={theme} color="#8b5cf6" icon="📅" label="Follow-ups" onClick={()=>nav("followups")} outline />}
          {cfg.features.map&&<QuickBtn theme={theme} color="#10b981" icon="🗺️" label="Beat Map" onClick={()=>nav("map")} outline />}
        </div>
      </div>
      <div style={{marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {stats.map(s=>(
            <div key={s.label} style={{background:theme.card,borderRadius:12,padding:"14px",border:`1px solid ${theme.border}`}}>
      ...
      <div style={{marginBottom:8}}>
        ...
        <div style={{background:theme.card,borderRadius:12,border:`1px solid ${theme.border}`,overflow:"hidden"}}>
          ...
          :recent.slice(0,5).map((item,i)=>{
            ...
            return (
              <div key={item.id} onClick={()=>nav(isHouse?"house-detail":"biz-detail",{id:item.id})} style={{padding:"12px 14px",borderBottom:i<4?`1px solid ${theme.border}`:"none",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
...
// ─── Add Entry ────────────────────────────────────────────────────────────────
function AddEntry({ nav, theme, params, toast, bump, cfg }) {
  const [tab,setTab]=useState(params.type||"house");
  const [areas,setAreas]=useState([]);
  const [saving,setSaving]=useState(false);
  const [gpsLoading,setGpsLoading]=useState(false);
  const editId=params.editId; const editType=params.editType;
  const [houseForm,setHouseForm]=useState({areaId:"",subArea:"",houseNumber:"",ownerName:"",mobileNumber:"",alternateNumber:"",familySize:1,addressNotes:"",deliveryNotes:"",customerStatus:cfg.statuses[0]?.id||"new",colorMarker:cfg.markerColors[0]?.id||"blue",gpsLat:null,gpsLng:null,photoUrl:null});
  const [bizForm,setBizForm]=useState({areaId:"",businessName:"",ownerName:"",mobileNumber:"",businessType:cfg.businessTypes[0]||"shop",interestedServices:[],followUpDate:"",remarks:""});
  useEffect(()=>{
  ...
  const handlePhoto=e=>{const file=e.target.files?.[0];if(!file)return;const r=new FileReader();r.onloadend=()=>{setHouseForm(f=>({...f,photoUrl:r.result}));toast("Photo captured!");};r.readAsDataURL(file);};
  const saveHouse=async()=>{
    ...
    try{await dbPut("houses",{...houseForm,id:editId||crypto.randomUUID(),createdAt:houseForm.createdAt||now,updatedAt:now});toast(editId?"House updated!":"House added!");bump();nav(editId?"house-detail":"dashboard",editId?{id:editId}:{});}
    catch{toast("Error saving","error");}setSaving(false);
  };
  const saveBiz=async()=>{
    ...
    setSaving(true);const now=new Date().toISOString();const id=editId||crypto.randomUUID();
    try{
      await dbPut("businesses",{...bizForm,id,createdAt:bizForm.createdAt||now,updatedAt:now});
      if(bizForm.followUpDate)await dbPut("followups",{id:crypto.randomUUID(),entityType:"business",entityId:id,entityName:bizForm.businessName,followUpDate:bizForm.followUpDate,notes:bizForm.remarks||"",completed:false,createdAt:now});
      toast(editId?"Business updated!":"Business added!");bump();nav(editId?"biz-detail":"dashboard",editId?{id:editId}:{});
    }catch{toast("Error saving","error");}setSaving(false);
  };
  ...
  return (
    <div>
      ...
      <div style={{padding:"0 16px 100px"}}>
        {!editId&&(
          <div style={{display:"grid",gridTemplateColumns:cfg.features.businesses?"1fr 1fr":"1fr",gap:8,marginBottom:20}}>
            {(cfg.features.businesses?["house","business"]:["house"]).map(t=>(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:12,padding:14}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {cfg.statuses.map(s=>(
                  <button key={s.id} onClick={()=>setHouseForm(f=>({...f,customerStatus:s.id}))} style={{padding:"10px 8px",borderRadius:8,border:`2px solid ${houseForm.customerStatus===s.id?s.color:theme.border}`,background:houseForm.customerStatus===s.id?s.color+"20":"transparent",color:houseForm.customerStatus===s.id?s.color:theme.muted,fontWeight:600,fontSize:13,cursor:"pointer"}}>
                    {s.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:12,padding:14}}>
              ...
              <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
                {cfg.markerColors.map(c=>(
                  <button key={c.id} onClick={()=>setHouseForm(f=>({...f,colorMarker:c.id}))} style={{width:34,height:34,borderRadius:"50%",background:c.hex,border:`3px solid ${houseForm.colorMarker===c.id?"#fff":"transparent"}`,outline:houseForm.colorMarker===c.id?`2px solid ${c.hex}`:"none",cursor:"pointer"}} title={c.id}/>
                ))}
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {cfg.features.gpsCapture&&(
              ...
              {cfg.features.photoCapture&&(
                <label style={{padding:"16px 8px",borderRadius:12,border:`1px solid ${houseForm.photoUrl?theme.success:theme.border}`,background:theme.card,color:houseForm.photoUrl?theme.success:theme.muted,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4,fontSize:12,fontWeight:600}}>
                  <span style={{fontSize:20}}>📷</span>{houseForm.photoUrl?"Photo ✓":"Take Photo"}
        ...
        {tab==="business"&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <FormInput label="Owner / Contact *" value={bizForm.ownerName} onChange={bf("ownerName")} theme={theme}/>
            <FormInput label="Mobile Number" value={bizForm.mobileNumber} onChange={bf("mobileNumber")} theme={theme} type="tel"/>
            <FormSelect label="Business Type" value={bizForm.businessType} onChange={bf("businessType")} theme={theme} options={cfg.businessTypes.map(t=>({value:t,label:t.charAt(0).toUpperCase()+t.slice(1)}))}/>
            <div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:12,padding:14}}>
...
function AreasPage({ nav, theme, refresh }) {
  const [areas,setAreas]=useState([]); const [houses,setHouses]=useState([]); const [businesses,setBusinesses]=useState([]); const [search,setSearch]=useState("");
  useEffect(()=>{ Promise.all([dbGetAll("areas"),dbGetAll("houses"),dbGetAll("businesses")]).then(([a,h,b])=>{setAreas(a);setHouses(h);setBusinesses(b);}); },[refresh]);
  ...
  return (
    <div style={{padding:"16px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:14}}>
        {filtered.map(area=>{
          const hCount=houses.filter(h=>h.areaId===area.id).length; const bCount=businesses.filter(b=>b.areaId===area.id).length;
          const total=hCount+bCount; const pct=Math.min(100,total>0?Math.round((total/Math.max(10,total+3))*100):0);
          return (
...
function SearchPage({ nav, theme, refresh }) {
  const [query,setQuery]=useState([]); const [houses,setHouses]=useState([]); const [businesses,setBusinesses]=useState([]);
  useEffect(()=>{ Promise.all([dbGetAll("houses"),dbGetAll("businesses")]).then(([h,b])=>{setHouses(h);setBusinesses(b);}); },[refresh]);
  const q=(query||"").toLowerCase();
  const fh=q?houses.filter(h=>h.ownerName?.toLowerCase().includes(q)||h.houseNumber?.toLowerCase().includes(q)||h.mobileNumber?.includes(q)):[];
  const fb=q?businesses.filter(b=>b.businessName?.toLowerCase().includes(q)||b.ownerName?.toLowerCase().includes(q)||b.mobileNumber?.includes(q)):[];
  return (
    <div style={{padding:16}}>
      ...
      <div style={{marginTop:16}}>
        {q&&fh.length===0&&fb.length===0&&<div style={{textAlign:"center",color:theme.muted,marginTop:40,fontSize:14}}>No results for "{q}"</div>}
        {fh.length>0&&(<div style={{marginBottom:16}}>
          ...
          {fh.map(h=>(
            <div key={h.id} onClick={()=>nav("house-detail",{id:h.id})} style={{background:theme.card,borderRadius:10,padding:"12px 14px",marginBottom:8,border:`1px solid ${theme.border}`,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontWeight:600,fontSize:13,color:theme.text}}>{h.ownerName||"Unknown"}</div><div style={{fontSize:11,color:theme.muted}}>House {h.houseNumber} • {h.mobileNumber}</div></div>
        ...
        {fb.length>0&&(<div>
          ...
          {fb.map(b=>(
            <div key={b.id} onClick={()=>nav("biz-detail",{id:b.id})} style={{background:theme.card,borderRadius:10,padding:"12px 14px",marginBottom:8,border:`1px solid ${theme.border}`,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontWeight:600,fontSize:13,color:theme.text}}>{b.businessName}</div><div style={{fontSize:11,color:theme.muted}}>{b.ownerName} • {b.mobileNumber}</div></div>
...
function ReportsPage({ nav, theme, refresh, toast }) {
  const [areas,setAreas]=useState([]); const [houses,setHouses]=useState([]); const [businesses,setBusinesses]=useState([]);
  useEffect(()=>{ Promise.all([dbGetAll("areas"),dbGetAll("houses"),dbGetAll("businesses")]).then(([a,h,b])=>{setAreas(a);setHouses(h);setBusinesses(b);}); },[refresh]);
  const exportCSV=()=>{
    let csv="HOUSES\nArea,House No,Owner,Mobile,Status\n";
    houses.forEach(h=>{const area=areas.find(a=>a.id===h.areaId)?.name||""; csv+=`${area},${h.houseNumber},${h.ownerName},${h.mobileNumber},${h.customerStatus}\n`;});
    csv+="\nBUSINESSES\nArea,Business Name,Owner,Mobile,Type\n";
    businesses.forEach(b=>{const area=areas.find(a=>a.id===b.areaId)?.name||""; csv+=`${area},${b.businessName},${b.ownerName},${b.mobileNumber},${b.businessType}\n`;});
    const blob=new Blob([csv],{type:"text/csv"}); const url=URL.createObjectURL(blob);
    const a=document.createElement("a"); a.href=url; a.download=`tensa-beat-${new Date().toISOString().split("T")[0]}.csv`; a.click(); URL.revokeObjectURL(url); toast("CSV exported!");
  };
  const activeAreas=areas.filter(a=>houses.some(h=>h.areaId===a.id)||businesses.some(b=>b.areaId===a.id));
  return (
    <div style={{padding:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div><div style={{fontSize:22,fontWeight:800,color:theme.primary,marginBottom:4}}>Reports</div><div style={{fontSize:12,color:theme.muted}}>Area & business statistics</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16}}>
        {[{label:"Houses",value:houses.length,icon:"🏠"},{label:"Businesses",value:businesses.length,icon:"🏪"},{label:"Prospects",value:houses.filter(h=>h.customerStatus==="prospect").length,icon:"🎯"}].map(s=>(
          <div key={s.label} style={{background:theme.card,borderRadius:12,padding:"12px 10px",border:`1px solid ${theme.border}`,textAlign:"center"}}>
            <div style={{fontSize:22}}>{s.icon}</div><div style={{fontSize:20,fontWeight:800,color:theme.text}}>{s.value}</div><div style={{fontSize:10,color:theme.muted}}>{s.label}</div>
          </div>
        ))}
      </div>
      <div style={{background:theme.card,borderRadius:12,border:`1px solid ${theme.border}`,marginBottom:14,overflow:"hidden"}}>
        ...
        {CONFIG.statuses.map(s=>{
          const count=houses.filter(h=>h.customerStatus===s.id).length; const pct=houses.length?Math.round((count/houses.length)*100):0;
          return (<div key={s.id} style={{padding:"10px 14px",display:"flex",alignItems:"center",gap:10,borderBottom:`1px solid ${theme.border}`}}>
            <div style={{width:10,height:10,borderRadius:"50%",background:s.color}}/><span style={{fontSize:13,flex:1,color:theme.text}}>{s.label}</span><span style={{fontSize:13,fontWeight:700,color:theme.text}}>{count}</span><span style={{fontSize:11,color:theme.muted}}>{pct}%</span>
          </div>);
        })}
      </div>
      {activeAreas.length>0&&(
        <div style={{background:theme.card,borderRadius:12,border:`1px solid ${theme.border}`,overflow:"hidden"}}>
          ...
          {activeAreas.map(a=>{const h=houses.filter(x=>x.areaId===a.id).length; const b=businesses.filter(x=>x.areaId===a.id).length;
            return (<div key={a.id} style={{display:"grid",gridTemplateColumns:"1fr auto auto",padding:"9px 14px",borderBottom:`1px solid ${theme.border}`}}>
              <span style={{fontSize:12,color:theme.text}}>{a.name}</span><span style={{fontSize:12,fontWeight:700,textAlign:"right",minWidth:30,color:theme.primary}}>{h}</span><span style={{fontSize:12,fontWeight:700,textAlign:"right",minWidth:30,color:theme.accent}}>{b}</span>
            </div>);
          })}
        </div>
      )}
...
function FollowUpsPage({ nav, theme, refresh, toast, bump }) {
  ...
  const today=new Date().toISOString().split("T")[0];
  const nextWeek=new Date(); nextWeek.setDate(nextWeek.getDate()+7); const nw=nextWeek.toISOString().split("T")[0];
  ...
  return (
    <div>
      ...
      <div style={{padding:"0 16px 80px"}}>
        <Grp title="⚠️ Overdue" items={pending.filter(f=>f.followUpDate<today)} color={theme.danger}/>
        <Grp title="📅 Today" items={pending.filter(f=>f.followUpDate===today)} color={theme.primary}/>
        <Grp title="📆 This Week" items={pending.filter(f=>f.followUpDate>today&&f.followUpDate<=nw)} color={theme.muted}/>
      </div>
    </div>
  );
}
...
// ─── Settings ─────────────────────────────────────────────────────────────────
function SettingsPage({ nav, theme, dark, setDark, toast, bump, cfg }) {
  const fileRef=useRef();
  const doExport=async()=>{ const json=await dbExport(); const blob=new Blob([json],{type:"application/json"}); const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download=`tensa-beat-backup-${new Date().toISOString().split("T")[0]}.json`; a.click(); URL.revokeObjectURL(url); toast("Backup exported!"); };
  const doImport=async e=>{ const file=e.target.files?.[0]; if(!file) return; try{const text=await file.text();await dbImport(text);toast("Database restored!");bump();setTimeout(()=>window.location.reload(),800);}catch{toast("Invalid backup file","error");} };
  return (
    <div>
      ...
      <div style={{padding:"0 16px 80px",display:"flex",flexDirection:"column",gap:20}}>
        ...
        <Sec title="DATA" theme={theme}>
          <SettingRow icon="⬇️" label="Export Backup" sub="Save full database as JSON" theme={theme} onClick={doExport}/>
          <SettingRow icon="⬆️" label="Restore Backup" sub="Load from JSON file" theme={theme} onClick={()=>fileRef.current?.click()} border={false}/>
          <input type="file" accept=".json" ref={fileRef} style={{display:"none"}} onChange={doImport}/>
        </Sec>
        <Sec title="CUSTOMISATION" theme={theme}>
          <SettingRow icon="⚙️" label="Admin Panel" sub="Edit title, colors, features" theme={theme} onClick={()=>nav("admin")} border={false}/>
        </Sec>
        <Sec title="ABOUT" theme={theme}>
          <SettingRow icon="ℹ️" label="Version" theme={theme}><span style={{fontSize:13,color:theme.muted}}>{cfg.app.version}</span></SettingRow>
...
function HouseDetail({ nav, theme, params, toast, bump }) {
  const [house,setHouse]=useState(null); const [area,setArea]=useState(null);
  useEffect(()=>{ if(!params.id)return; dbGet("houses",params.id).then(h=>{setHouse(h);if(h)dbGet("areas",h.areaId).then(setArea);}); },[params.id]);
  const del=async()=>{ if(!confirm("Delete this house record?"))return; await dbDelete("houses",house.id); toast("House deleted");bump();nav("dashboard"); };
    ...
    <div>
      ...
      <div style={{padding:"16px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        {(house.mobileNumber||house.alternateNumber)&&(
          <InfoCard theme={theme}>
            {house.mobileNumber&&(<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${theme.border}`}}>
            ...
            {house.alternateNumber&&(<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0"}}>
        ...
        {(house.addressNotes||house.deliveryNotes)&&(<InfoCard theme={theme}>
        ...
        {(house.gpsLat&&house.gpsLng)&&(<a href={`https://maps.google.com?q=${house.gpsLat},${house.gpsLng}`} target="_blank" rel="noreferrer" style={{background:theme.primary+"15",border:`1px solid ${theme.primary}44`,borderRadius:12,padding:14,display:"flex",alignItems:"center",gap:10,textDecoration:"none",color:theme.primary,fontWeight:700,fontSize:14}}>
          📍 View on Google Maps <span style={{fontSize:11,color:theme.muted,fontWeight:400}}>{house.gpsLat.toFixed(5)}, {house.gpsLng.toFixed(5)}</span>
        </a>)}
        ...
        <div style={{fontSize:11,color:theme.muted,textAlign:"center"}}>Added {new Date(house.createdAt).toLocaleDateString()}</div>
      </div>
    </div>
  );
}
...
function BizDetail({ nav, theme, params, toast, bump }) {
  const [biz,setBiz]=useState(null); const [area,setArea]=useState(null);
  useEffect(()=>{ if(!params.id)return; dbGet("businesses",params.id).then(b=>{setBiz(b);if(b)dbGet("areas",b.areaId).then(setArea);}); },[params.id]);
  const del=async()=>{ if(!confirm("Delete this business?"))return; await dbDelete("businesses",biz.id); toast("Business deleted");bump();nav("dashboard"); };
  ...
  return (
    <div>
      ...
      <div style={{padding:"16px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <InfoCard theme={theme}><InfoRow label="Area" value={area?.name||"—"} theme={theme}/><InfoRow label="Owner" value={biz.ownerName} theme={theme} border={false}/></InfoCard>
        {biz.mobileNumber&&(<div style={{background:theme.card,border:`1px solid ${theme.border}`,borderRadius:12,padding:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        {biz.interestedServices?.length>0&&(<InfoCard theme={theme}>
          <div style={{fontSize:11,color:theme.muted,marginBottom:8}}>INTERESTED SERVICES</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{biz.interestedServices.map(s=><span key={s} style={{background:theme.primary+"18",color:theme.primary,padding:"4px 10px",borderRadius:6,fontSize:12,fontWeight:600}}>{s}</span>)}</div>
        </InfoCard>)}
        {biz.followUpDate&&(<div style={{background:"#f59e0b22",border:"1px solid #f59e0b44",borderRadius:12,padding:14,display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:20}}>📅</span><div><div style={{fontSize:11,color:theme.muted}}>FOLLOW-UP DATE</div><div style={{fontSize:14,fontWeight:700,color:"#f59e0b"}}>{biz.followUpDate}</div></div>
        </div>)}
...
function StatusBadge({status}){const c=STATUS_COLOR[status]||"#888";return <span style={{background:c+"22",color:c,padding:"3px 10px",borderRadius:6,fontSize:11,fontWeight:700,textTransform:"capitalize"}}>{status}</span>;}
